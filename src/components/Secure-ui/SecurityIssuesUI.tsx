...
const IssueCard: React.FC<{ issue: Issue }> = ({ issue }) => (
  <Card className="bg-white dark:bg-gray-900 border dark:border-gray-700 mt-4 p-4 space-y-2 shadow-md">
    ...
    <p className="text-sm text-gray-800 dark:text-gray-300">
      <strong>Recommendation:</strong> {issue["Security Recommendations"]}
    </p>
    {issue["Corrected example code"] && (
      <div className="relative">
        <button
          className="absolute top-1 right-1 bg-gray-200 dark:bg-gray-700 hover:bg-gray-300 dark:hover:bg-gray-600 text-xs px-2 py-1 rounded"
          onClick={() =>
            navigator.clipboard.writeText(issue["Corrected example code"] || "")
          }
        >
          Copy
        </button>
        <pre className="text-xs font-mono p-2 rounded w-full bg-gray-100 dark:bg-gray-800 whitespace-pre overflow-x-auto text-gray-900 dark:text-gray-200">
          {issue["Corrected example code"]}
        </pre>
      </div>
    )}
...
const ChunkView: React.FC<{ chunk: Chunk }> = ({ chunk }) => (
  <div className="mt-4">
    ...
    <div className="relative">
      ...
      <pre className="bg-gray-100 dark:bg-gray-800 p-2 rounded text-xs overflow-x-auto max-h-64 text-gray-900 dark:text-gray-200">
        {chunk.chunk_content}
      </pre>
    </div>
    {chunk.ai_issues.map((issue, idx) => (
...
const FileAccordion: React.FC<{
  ...
  const totalIssues = fileData?.chunks?.reduce(
    (acc, chunk) => acc + chunk?.ai_issues?.length,
    0
  );

  return (
    <Card className="border dark:border-gray-700 mb-4 page-break-avoid">
      <div
        className="p-4 cursor-pointer bg-blue-50 dark:bg-blue-900 hover:bg-blue-100 dark:hover:bg-blue-800"
        onClick={() => setOpen(!open)}
      >
        <div className="flex justify-between items-center">
